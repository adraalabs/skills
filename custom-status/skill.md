...
Tool: `store_data` key:"bio:{caller_id}" value:"{bio}"
Template: "Bio updated!"

## Command: /setstatus

Options: status (string, required)
Tool: `store_data` key:"status:{caller_id}" value:"{status}"
...
Options: user (user)
Steps:
1. `get_data` key:"bio:{target_id}" name:"bio"
2. `get_data` key:"status:{target_id}" name:"st"
3. `get_position` prefix:"xp:" key:"xp:{target_id}" name:"rank"
