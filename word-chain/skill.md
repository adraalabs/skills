...
Actions:
1. `get_data` key:"wordchain_last" — get the last valid word
2. Check if {message.content} starts with the last letter of the previous word
3. Check if the word was already used: `get_data` key:"wordchain_used:{message.content}"
4. If valid: `store_data` key:"wordchain_last" value:"{message.content}", `store_data` key:"wordchain_used:{message.content}" value:true, `add_reaction` ✅
5. If invalid: `send_message` "Wrong! The word must start with the letter **X**", `store_data` key:"wordchain_last" value reset

## Command: /wordchain reset

`delete_data_by_prefix` prefix:"wordchain_"
